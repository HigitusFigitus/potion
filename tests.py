...
class TestCase(unittest.TestCase):
  ...
  def test_post_valid_potion(self):
    params = json.dumps(dict(potion_name = "potion_1",
                             potion_type= "passive",
                             potion_class= "life"))
    response = self.app.post("/api/v1/potions", headers=headers, data=params)
  ...
  def test_post_missing_potion_type(self):
    params = json.dumps(dict(potion_name = "potion_1", potion_class = "poison"))
    response = self.app.post("/api/v1/potions", headers=headers, data=params)
    self.assertEqual(response.status_code, 400)
    data = json.loads(response.data)
  ...
  def test_post_missing_potion_class(self):
    params = json.dumps(dict(potion_name = "potion_1", potion_type = "passive"))
    response = self.app.post("/api/v1/potions", headers=headers, data=params)
    self.assertEqual(response.status_code, 400)
    data = json.loads(response.data)
