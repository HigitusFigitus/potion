...
def potions():
  if request.method == "POST":
    ...
    potions_filter = request.args
    if "potion_type" in potions_filter and potions_filter["potion_type"] in (
                                                      "passive", "active"):
      potions = Potion.query.filter_by(
        potion_type=potions_filter["potion_type"]).all()

    elif "potion_class" in potions_filter and potions_filter["potion_class"] in (
                                          "life", "mana", "fire", "poison"):
      potions = Potion.query.filter_by(
        potion_class=potions_filter["potion_class"]).all()
    else:
